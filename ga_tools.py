...
class GA(Agent):
    ...
    def __init__(self, clients, solution, num_generations, num_parents_mating):
        GA.unique_id += 1
        self.unique_id = GA.unique_id
    ...
    def step(self):
        ...
        for generation in range(self.num_generations):
            ...
            for i in range(pop_size):
                fitness[i] = frt.simulate(self.clients, new_pop[i])
            print('\033[1m'+"Gen ", generation, " :" + '\033[0m')
            best_fit = min(fitness)
            print("Best result: ", best_fit)
            local_best_fit_list.append(best_fit)
            
            # select the best parents in the population for mating
            parents = self.mating_pool(new_pop, fitness, self.num_parents_mating)
            
            # generate next pop using crossover
            offspring_co = self.crossover(parents=parents, offspring_size=(pop_size - parents.shape[0]), num_genes=num_genes)
            
            # add some random variations (mutations)
            offspring = self.mutation(offspring_co)
        ...
        self.solution = new_pop[np.where(fitness == np.min(fitness))[0][0], :]
        self.best_fit_list = local_best_fit_list
    ...
    def mating_pool(self, new_pop, fitness, num_parents_mating):
    ...
    def crossover(self, parents, offspring_size, num_genes):
    ...
    def mutation(self, offspring_co):
