...
#   solutions, using the principle of crossover to mix half the genes of each parent
def crossover(parents, offspring_size, num_genes):
    # usually the crossover divides the chromossomes in the middle
    crossover_p = np.uint8(num_genes/2)
    ...
    for k in range(offspring_size):
        ...
        # 1st half of genes comes from parent 1 and the other comes from parent 2
        offspring[k, 0: crossover_p] = parents[parent_idx[0], 0: crossover_p]
        offspring[k, crossover_p:] = parents[parent_idx[1], crossover_p :]
...
def mutation(offspring_co):
    ...
    for idx in range(offspring_co.shape[0]):
        # the 1st random idx to be swapped
        random_id1 = np.random.randint(0, offspring_co.shape[0]-1)
        
        # the 2nd random idx to be swapped
        random_id2 = np.random.randint(0, offspring_co.shape[0]-1)
